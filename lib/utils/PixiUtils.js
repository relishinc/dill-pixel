...
export function makeSprite(pAsset, pSheet) {
    const sprite = new Sprite(getTexture(pAsset, pSheet));
...
export function makeSpine(pName) {
    const spine = new Spine(Assets.get(pName).spineData);
...
export function scaleUniform(obj, scaleNum, scaleProp = 'width') {
    const scaleVal = scaleProp === 'width' ? 'y' : 'x';
    const otherScaleVal = scaleProp === 'width' ? 'x' : 'y';
    obj[scaleProp] = scaleNum;
    obj.scale[scaleVal] = obj.scale[otherScaleVal];
}
export function scaleToWidth(obj, width) {
    scaleUniform(obj, width, 'width');
}
export function scaleToHeight(obj, height) {
    scaleUniform(obj, height, 'height');
}
export function scaleToSize(obj, size, firstProp = 'width') {
