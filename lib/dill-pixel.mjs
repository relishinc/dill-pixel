import { Point as T, isMobile as Qe, Assets as p, Container as K, Graphics as Ae, Texture as gt, Sprite as ke, Text as mt, BitmapText as vt, Rectangle as Be, EventEmitter as X, Ticker as V, DOMAdapter as yt, path as ee, ExtensionType as Ue, LoaderParserPriority as wt, extensions as bt, Application as xt } from "pixi.js";
...
let _ = m;
const Bt = (i = 0) => new Promise((e) => setTimeout(e, i * 1e3)), Je = (i) => i && typeof i.then == "function";
function Ut(...i) {
  return new $t(i);
}
class $t {
...
class Qt extends E {
  ...
  initialize(e, t) {
    t != null && t.required && (this._required = t.required), t != null && t.background && (this._background = t.background), _.log("AssetsPlugin initialized", e, t, this._required);
  }
  async loadRequired() {
  ...
  loadBackground() {
    console.log("loadBackground", this._background, p), this._background && (this._background.assets && p.backgroundLoad(this._background.assets), this._background.bundles && p.backgroundLoadBundle(this._background.bundles));
  }
  async loadAssets(e) {
  ...
  async unloadSceneAssets(e) {
    return e.assets && p.unload(e.assets), e.bundles && p.unloadBundle(e.bundles), Promise.resolve();
  }
  async loadSceneAssets(e, t = !1) {
    t ? (e.backgroundAssets || e.backgroundBundles) && (e.backgroundAssets && p.backgroundLoad(e.backgroundAssets), e.backgroundBundles && p.backgroundLoadBundle(e.backgroundBundles)) : (this._handleLoadStart(), this._handleLoadProgress(0), e.assets && await p.load(e.assets, this._handleLoadProgress), e.bundles && await p.loadBundle(e.bundles, this._handleLoadProgress), this._handleLoadComplete());
  }
  getCoreFunctions() {
...
class Jt extends E {
  ...
  async loadScene(e) {
    ...
    switch (this._currentSceneId = t, this._queue = Ut(this._createCurrentScene), s) {
      case "exitEnter":
        this._queue.add(
          this._exitLastScene,
  ...
  async _createCurrentScene() {
    ...
    if (!t)
      throw new Error(`Couldn't load ${this._currentSceneId}"`);
    this.currentScene = new t(), this.currentScene.id = this._currentSceneId, e.bundles !== void 0 && (this.currentScene.bundles = e.bundles), e.assets !== void 0 && (this.currentScene.assets = e.assets), e.backgroundAssets !== void 0 && (this.currentScene.backgroundAssets = e.backgroundAssets), e.backgroundBundles !== void 0 && (this.currentScene.backgroundBundles = e.backgroundBundles), e.autoUnloadAssets !== void 0 && (this.currentScene.autoUnloadAssets = e.autoUnloadAssets), this.onSceneChangeStart.emit({ exiting: ((s = this._lastScene) == null ? void 0 : s.id) || null, entering: this.currentScene.id });
  }
  _queueComplete() {
...
class ts extends E {
  ...
  _onGamepadDisconnected(e) {
    this._deactivateGamepad(e.gamepad.id), this.sendAction(x.Pause), this.onGamepadDisconnected.emit(e.gamepad), this.activeGamepads.size === 0 && this._deactivateController(B.Gamepad);
  }
}
class ss extends E {
  constructor() {
...
class ti extends $ {
  constructor() {
    super({ autoResize: !0, autoUpdate: !0, priority: -9999 }), this.autoUnloadAssets = !1, this._assets = null, this._bundles = null, this._backgroundAssets = null, this._backgroundBundles = null;
  }
  get assets() {
    return this._assets;
  }
  ...
  set backgroundAssets(e) {
    this._backgroundAssets = e;
  }
  get backgroundBundles() {
    return this._backgroundBundles;
  }
  set backgroundBundles(e) {
    this._backgroundBundles = e;
  }
  async initialize() {
  }
  /**
...
const U = class {
   ...
   */
  _poolInstance(i) {
    i.destroy(), U._pool.indexOf(i) < 0 && U._pool.push(i);
  }
};
let ae = U;
ae._pool = [];
...
class g extends xt {
  ...
  async initialize(e) {
    ...
    if (g.instance = this, this.config = Object.assign({ ...Us }, e), e.container && (g.containerElement = e.container), _.initialize(this.config.logger), await this.boot(this.config), await this.preInitialize(this.config), await this.initAssets(), await this.init(this.config), await this.registerDefaultPlugins(), this.config.plugins && this.config.plugins.length > 0)
