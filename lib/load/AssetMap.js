...
export class AssetMap {
    static { this._map = new Dictionary(); }
    static { this._debug = process.env.NODE_ENV === 'development'; }
    /**
    ...
    static getAssetGroup(pGroupId) {
        if (this._map.size() === 0) {
            ...
            if (assets === undefined) {
                this.logW('%c%s%c asset group has not been added to the map. Call addAssetGroup first. Returning ' + 'empty array.', LogUtils.STYLE_RED_BOLD, pGroupId, LogUtils.STYLE_WHITE);
                return new Array();
            }
            else {
                return assets;
