...
export class KeyboardManager {
    ...
    removeKeyBinding(pDirection, pKeyCode, pModifiers) {
        ...
        if (found) {
            ...
            this.log(`removeKeyBinding: Key "${KeyboardManager.bindingToString(Object.assign({ code: pKeyCode }, pModifiers))}" is no longer mapped to "${pDirection.toString()}"`);
        }
        else if (this.isKeyBound(pKeyCode, pModifiers)) {
            this.logE(`removeKeyBinding: Key "${KeyboardManager.bindingToString(Object.assign({ code: pKeyCode }, pModifiers))}" was not mapped to mapped to direction "${pDirection.toString()}", so nothing has been removed.`);
        }
        else {
            this.logE(`removeKeyBinding: Key "${KeyboardManager.bindingToString(Object.assign({ code: pKeyCode }, pModifiers))}" is not mapped to any direction, so nothing has been removed.`);
        }
    }
    ...
    onRegisterFocusable(pData) {
        if (this._maps.length > 0) {
            this._maps[0].registerFocusable(pData);
        }
    }
    onRegisterFocusables(pData) {
        if (this._maps.length > 0) {
            this._maps[0].registerFocusable(pData);
        }
    }
    onUnregisterFocusable(pData) {
        for (const map of this._maps) {
            map.unregisterFocusable(pData);
        }
    }
    onUnregisterFocusables(pData) {
        for (const map of this._maps) {
            map.unregisterFocusable(pData);
        }
    }
