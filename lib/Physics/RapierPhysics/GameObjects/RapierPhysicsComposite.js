...
export class RapierPhysicsComposite extends Container {
    ...
    addVisual(color, size, position = [0,
        0], type = PhysicsBodyType.RECTANGLE) {
    ...
    createPiece(color, size, position = [0,
        0], angle = 0, type = PhysicsBodyType.RECTANGLE, data) {
    ...
    update() {
        if (this.visuals && this.body) {
            this.colliderRef.forEach(({ collider, visual, data }) => {
                ...
                if (data === null || data === void 0 ? void 0 : data.isMain) {
...
RapierPhysicsComposite.DEFAULT_DEBUG_COLOR = 0x29c5f6;
