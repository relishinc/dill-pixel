...
2. Create a `vite.config.js` file in the root directory to configure the build process. The configuration
is fairly standard but we're using the [vite-plugin-static-copy](https://www.npmjs.com/package/vite-plugin-static-copy)
to copy our static assets so that they can be loaded from within the game. You can configure the plugin
