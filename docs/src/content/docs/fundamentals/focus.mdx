...
init(){
    addKeyboardLayer(); // usually called in the init method of your state

    // ... later on, create and register some focusable elements
...
destroy(){
    removeKeyboardLayer();
    super.destroy() // call the super method or your state will not be destroyed
}
```
...
## Example
To see all this in action, check out the [Button Example](https://dillpixel.io/examples/#ButtonExample), the
