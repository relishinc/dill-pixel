...
import {AssetMap} from "../Load/AssetMap";
 ...
 */
export class StateManager extends PIXI.Container {
    /**
     ...
     */
    private _currStateId: string = "undefined";
    /**
     * The new state.
     */
     ...
     */
    private _currStateData: IStateData | undefined;
    /**
    ...
    constructor() {
        ...
        PubSub.subscribe(Topics.STATE_LOAD_STATE, this.onStateLoadRequested.bind(this));
    }
    ...
    public update(pDeltaTime: number): void {
        if (this._currState !== undefined) {
            this._currState.update(pDeltaTime);
        }
    }
    ...
    public onResize(pSize: PIXI.Point): void {
        ...
        if (this._currState !== undefined) {
            this._currState.onResize(this._size);
        }
    }
    ...
    private startTransition(pStateId: string, pLoadScreen?: string): void {
        this.log("Transition requested from %c%s %cto %c%s.", LogUtils.STYLE_RED_BOLD,
            this._currStateId, LogUtils.STYLE_BLACK, LogUtils.STYLE_RED_BOLD,
            pStateId);
    ...
    private handleLoadAssetsComplete(): void {
        ...
        if (this._newStateToken !== undefined && this._newStateToken.transitionSteps.length > 0 &&
    ...
    private setAndPerformTransitionStep(pStepIndex: number): void {
        ...
        if (this._transitionStepIndex < this._newStateToken!.transitionSteps.length) {
            ...
            this._currStateData = this._newStateData;
            this._currStateId = this._newStateToken!.stateId;
    ...
    private performTransitionStep(pStep: TransitionStep): void {
        this.log("performTransitionStep(%c%s%c)",
            LogUtils.STYLE_BLUE_BOLD, TransitionStep[pStep], LogUtils.STYLE_BLACK);
    ...
    private performStepAnimNewIn(): void {
        ...
        if (this._newState === undefined) {
            this.logE("The new view is null. Ensure step %s or %s is performed before this one.",
                TransitionStep.AttachNewInFront, TransitionStep.AttachNewBehind);
            this.performNextTransitionStep();
        } else {
    ...
    private performStepAnimCurrentOut(): void {
        ...
        if (this._currState === undefined) {
            ...
            this._currState.animateOut(this.handleViewAnimateOutComplete.bind(this));
        }
    }
    ...
    private performStepAttachNewView(pNewInFront: boolean): void {
        ...
        this.log("(%s)\n --- current view: '%s' @ time %s with %s milliseconds",
            this._newStateToken!.stateId, this._currStateId, d.toLocaleTimeString(), d.getMilliseconds());
        ...
        if (this._newState !== undefined) {
            ...
            if (pNewInFront) {
                this.log("Attaching %c%s%c in %cfront.", LogUtils.STYLE_RED_BOLD, this._newStateToken!.stateId,
                    LogUtils.STYLE_BLACK, LogUtils.STYLE_BLUE_BOLD);
                this.setChildIndex(this._newState, this.children.length - 1);
            } else {
                this.log("Attaching %c%s%c %cbehind.", LogUtils.STYLE_RED_BOLD, this._newStateToken!.stateId,
                    LogUtils.STYLE_BLACK, LogUtils.STYLE_BLUE_BOLD);
    ...
    private performStepRemoveCurrent(): void {
        ...
        if (this._currState === undefined) {
            ...
            // Remove from stage.
            this.removeChild(this._currState);

            // State will not be reused so destroy it.
            this._currState.destroy({children: true});

            // Notify the world when a view is removed.
            // _mediator.NotifyViewRemoved(_currentView.Token);
            this._currState = undefined;
            delete this._currState;

            this.log("Current %c%s%c removed.", LogUtils.STYLE_RED_BOLD, this._currStateId, LogUtils.STYLE_BLACK);
    ...
    private performStepLoadAssets(): void {
        ...
        if (assetsToLoad.length > 0) {
            ...
            PubSub.publishSync(Topics.LOAD_ASSETS, token);
        } else {
            // No assets to load.
            this.logW("No new assets to load found for %c%s%c. Advancing transition.", LogUtils.STYLE_RED_BOLD,
                this._newStateToken!.stateId, LogUtils.STYLE_BLACK);
            this.performNextTransitionStep();
        }
    }
    ...
    private performStepUnloadAssets(): void {
        ...
        const assets: AssetMapData[] = this.trimUnloadList(this._currStateId, this._newStateToken!.stateId);

        if (assets !== undefined) {
            if (assets.length > 0) {
                const token: LoadToken = new LoadToken(assets, this.handleUnloadAssetsComplete.bind(this),
                    this._loadScreen);
                PubSub.publishSync(Topics.UNLOAD_ASSETS, token);
            } else {
                // No assets to unload
                this.performNextTransitionStep();
            }
    ...
    private performStepShowLoadScreen(): void {
        ...
        if (this._loadScreen !== undefined) {
            PubSub.publishSync(Topics.SHOW_LOAD_SCREEN, {
    ...
    private performStepHideLoadScreen(): void {
        ...
        if (this._loadScreen !== undefined) {
            PubSub.publishSync(Topics.HIDE_LOAD_SCREEN, {
                callback: this.handleLoadScreenAnimateOutComplete.bind(this), loadScreen: this._loadScreen,
            });
        } else {
            this.logE("Load screen show requested with %c%s%c passed in. Please specify a registered load screen to" +
                " use.", LogUtils.STYLE_RED_BOLD, this._loadScreen, LogUtils.STYLE_BLACK);
    ...
    private performStepHalt(): void {
        ...
        PubSub.publishSync(Topics.STATE_TRANSITION_HALTED, this._newStateToken);
    }
    ...
    private trimUnloadList(pOldStateId: string, pNewStateId: string): AssetMapData[] {
        ...
        const assetsToKeep: string[] = requiredAssets.map((a) => a.assetName)
