...
export class KeyboardManager {
  ...
  public addKeyBinding(
    ...
    if (existingDirection !== false) {
      if (existingDirection.direction === pDirection) {
        this.log(
          `addKeyBinding: Ignoring duplicate mapping. "${KeyboardManager.bindingToString(
            binding,
          )}" is already mapped to "${pDirection.toString()}"`,
        );
        return;
      } else {
        this.logW(
          `addKeyBinding: Key "${KeyboardManager.bindingToString(
            binding,
  ...
  public removeKeyBinding(
    ...
    if (found) {
      ...
      this.log(
        `removeKeyBinding: Key "${KeyboardManager.bindingToString({
          code: pKeyCode,
          ...pModifiers,
  ...
  private onForceNeighbours(pData: KeyboardMapToken | KeyboardMapToken[]) {
    if (this._maps.length > 0) {
  ...
  private onGetKeyboardStatus(pData: (status: IKeyboardStatus) => void) {
    pData({
