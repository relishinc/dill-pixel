...
// 1 param
     ...
     */
    playVO(key: string | Array<string | number>): void;
// 2 params
    /**
     * Play a Voiceover or sequence of voiceovers
     * @param key Id or array of voiceover Ids
     ...
     */
    playVO(key: string | Array<string | number>, mode: PlayMode): void;
    /**
     * Play a Voiceover or sequence of voiceovers
     * @param key Id or array of voiceover Ids
     * @param callback Called after the last voiceover finishes playing, or immediately if no playback occurs
     */
    playVO(key: string | Array<string | number>, callback: Callback): void;
    /**
     * Play a Voiceover or sequence of voiceovers
     * @param key Id or array of voiceover Ids
     ...
     */
    playVO(key: string | Array<string | number>, options: IPlayOptions): void;
// 3 params
    /**
     * Play a Voiceover or sequence of voiceovers
     * @param key Id or array of voiceover Ids
     ...
     * @param callback Called after the last voiceover finishes playing, or immediately if no playback occurs
     */
    playVO(key: string | Array<string | number>, mode: PlayMode, callback: Callback): void;
...
export class VoiceOverManager implements IVoiceOverManager {
    ...
    public playVO(
        ...
        this.log("🗣 Play VO requested. Key(s): [%c%s%c], Mode: %c%s%c, Priority: %c%s%c",
            RED, key.join(", "), BLACK,
            BLUE, mode, BLACK,
            BLUE, priority, BLACK,
        );
    ...
    private addToQueue(keys: Array<string | number>, cb?: Callback, skipCC?: boolean, priority?: number, caption?: {id: string, args: {[key: string]: string}}) {
    ...
    private playNext() {
        if (this._queue.length > 0) {
            ...
            if (item.delay !== undefined) {
                ...
                if (this._queue.length === 1) { // skip delay if last item in queue
                ...
                Application.instance.audio.load(item.key, AudioCategory.VO, () => {
                    ...
                    if (!item.skipCC) {
                        // TODO: Captions are not actually part of the framework
                        if (item.caption) {
                            PubSub.publishSync(Topics.PLAY_CAPTION, {id: item.caption.id, args: item.caption.args});
                        }
                        else {
    ...
    private log = (m: string, ...params: any[]) => {
        if (this.debug) { LogUtils.log(m, {className: "VoiceOverManager", color: "salmon"}, ...params); }
    }
    private warn = (m: string, ...params: any[]) => {
        LogUtils.logWarning(m, {className: "VoiceOverManager", color: "salmon"}, ...params);
    }
    private error = (m: string, ...params: any[]) => LogUtils.logError(m, {className: "VoiceOverManager", color: "salmon"}, ...params);
}
