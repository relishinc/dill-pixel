 ...
 */
export class LoadManager extends PIXI.Container {
    /**
     ...
     */
    constructor(pSplashScreen: SplashScreen) {
        super();
        ...
        PubSub.subscribe(Topics.SHOW_LOAD_SCREEN, this.showLoadScreen);
        PubSub.subscribe(Topics.HIDE_LOAD_SCREEN, this.hideLoadScreen);
        PubSub.subscribe(Topics.LOAD_ASSETS, this.onLoadRequested);
        PubSub.subscribe(Topics.UNLOAD_ASSETS, this.onUnloadRequested);
    }
     ...
     */
    public startSplashProcess(pPersistentAssets: AssetMapData[], pOnComplete: () => void): void {
        // Remove assets from the persistent list that will already be loaded by the splash screen.
        const splashAssets: AssetMapData[] = AssetMap.getAssetGroup(SplashScreen.NAME);
     ...
     */
    public registerLoadScreen(pKey: string, pScreen: LoadScreenProvider, pDefault: boolean = false) {
     ...
     */
    private showLoadScreen(pTopic: string, pData: { loadScreen: string, stateData?: any, callback: () => void }): void {
     ...
     */
    private hideLoadScreen(pTopic: string, pData: { loadScreen: string, callback: () => void }): void {
     ...
     */
    private load(pAssets: AssetMapData[] | undefined, pOnComplete: () => void): void {
        if (this._onLoadComplete !== undefined) {
            this.logW("Replacing existing _onLoadComplete callback!");
        }
        this.log("Asset Load requested. Count: %c%s",
            LogUtils.STYLE_RED_BOLD, pAssets === undefined ? "Nothing to load" : pAssets.length);
    ...
    private loadOnSplash(pAssets: AssetMapData[], pOnComplete: () => void): void {
        ...
        this.log("Splash Asset Load requested. Count: %c%s",
            LogUtils.STYLE_RED_BOLD, pAssets === undefined ? "Nothing to load" : pAssets.length);
        this._onLoadComplete = pOnComplete;
     ...
     */
    private unload(pAssets: AssetMapData[] | undefined, pOnComplete?: () => void): void {
     ...
     */
    private getLoadScreen(pLoadScreen: string | undefined): LoadScreen | undefined {
        this._isCurrentLoadScreenPersistent = true;
        if (pLoadScreen !== undefined) {
            let loadScreen: LoadScreenProvider | undefined = this._loadScreens.getValue(pLoadScreen);
     ...
     */
    private async loadAssets(pAssets: AssetMapData[] | undefined): void {
        if (pAssets !== undefined) {
            ...
            if (audioAssets.length > 0) {
                Assets.load(audioAssets.map(asset => asset.getResource().src), this.onPixiLoadProgress).then(() => {
                    PubSub.publishSync(Topics.LOAD_AUDIO_FROM_ASSET_MAP, {
                        assets: audioAssets,
                        callback: () => { // TODO: refactor nested callback
                            this.log("Audio load complete.");
                            this.onAllLoadsComplete();
                        },
                        progressCallback: this.onAudioLoadProgress.bind(this),
                    });
                }).catch(e => this.onPixiLoadError(e));

            } else {
                this.log("No AUDIO to load");
    ...
    private removeRedundantLoads(pAssets: AssetMapData[]): AssetMapData[] {
        ...
        for (let i = 0; i < pAssets.length; ++i) {
            ...
            for (let j = 0; j < prunedAssets.length; j++) {
                ...
                if (assetInList.assetName === curAsset.assetName && assetInList.resolutionSuffix === curAsset.resolutionSuffix) {
                    isFound = true;
                    break;
                }
            }
            ...
            if (!isFound) {
                this.log("Adding %c%s%c to the final list.",
                    LogUtils.STYLE_RED_BOLD, curAsset.assetName, LogUtils.STYLE_BLACK);
                prunedAssets.push(curAsset);
            } else {
                this.log("Excluding %c%s%c from the final list because it is already included.",
                    LogUtils.STYLE_RED_BOLD, curAsset.assetName, LogUtils.STYLE_BLACK);
            }
        }
        return prunedAssets;
    ...
    private onLoadAnimateOutComplete(): void {
        ...
        PubSub.publishSync(Topics.LOAD_SCREEN_HIDDEN, undefined);
    }
