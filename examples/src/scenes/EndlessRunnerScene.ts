...
export class EndlessRunnerScene extends BaseScene {
	...
	update(ticker: Ticker) {
		...
		if (this.app.keyboard.isKeyDown('ArrowLeft') || this.app.keyboard.isKeyDown('a') || this._joystick.direction.includes('left')) {
			this.app.sendAction('move_left');
		}
		if (this.app.keyboard.isKeyDown('ArrowRight') || this.app.keyboard.isKeyDown('d') || this._joystick.direction.includes('right')) {
	...
	addControls() {
		...
		this.ui.addElement(this._joystick, {align: 'bottom left'});
		this.ui.addElement(jumpButton, {align: "bottom right", padding: {right: 15, bottom: 15}})
	}
