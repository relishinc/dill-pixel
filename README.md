...
Relish framework for HTML game development.
...
### Documentation
* Available here: [Docs]
* Documentation is a work in progress and is incomplete. Speak to Jack or Rick if the information needed is not present.
...
### Release Process
##### Feature development
1. Develop features in seperate branches in the feature folder.
2. When a feature is completed, create a PR for others to review.
3. When PR is approved, merge develop into the feature branch, fix any merge issues and combine both Unreleased sections of the changelog.
4. Finish the PR.
##### New Version Release
1. When a release is ready, create a new release section in the changelog with everything from the Unreleased section.
2. Update the package version in `package.json` with the new version number.
3. Commit the changelog and the version number change.
4. Merge develop into master.
5. Merge master into release and create a version number tag for the commit.
6. Merge release into master.

### Who do I talk to? ###

Jack Ront or Rick Mason

#### Changelog
Updates and changes should all be noted and explained in the `CHANGELOG.md` file. All instructions for format and structure are linked in that file.

#### NPM
The **node_modules** folder is ignored in git because it is massive and pointless to store. So, in order to run the project on any computer, `npm install` must be run first. It should be run as soon as the project is cloned. This ensures that intellisense will not complain about missing things and when the project is run, it will actually run. Commands available using `npm foo` are located in the `package.json` file, in the `scripts` section near the top. All commands will be described in the `package.json` section below.
...
#### FFMPEG
* Audio compression requires that FFMPEG is installed. If you use Homebrew on OS X, this is as easy as: `brew install ffmpeg`. If you are on Windows, download FFMPEG [here][ffmpeg download].
* On Mac OS, the audio compression script in your project will also need executable permissions. Open a terminal and go to the `src/audio` directory in the project. `ls -lisa` will likely show the permissions on `compress_audio.sh` as `-rw-r--r--`. If this is the case run `chmod 755 compress_audio.sh` to make it executable.
...
```
* `HLF.Application.createSplashScreen` should be overridden to customize the splashscreen. The splashscreen returned will need to inherit from `HLF.Splashscreen`.
...
#### Audio
* All audio files need to be in the `src/audio/source` folder. Any folder structure beyond that is purely organizational and will not affect the outcome of the `audio` command. This means that all audio files need to have a unique name. Currently, all audio tracks must be in the `.wav` format.
...
* There is a free alternative for Windows called [BM Font Generator][bm-font-generator]. I have not tried this alternative out.
...
* Two folders:
    * `spritesheets`
        * This is where you organize your sheets. For each spritesheet you should have a folder of the source assets for the sheet and the texture packer project file (`.tps`)


        ![spritesheet-source](http://hlf.reli.sh/assets/images/spritesheet-source.png)


        * The project should output to the `spritesheets/_output` folder with the name `foo{v}.json`. The `{v}` will be replaced by either the `@2x` or `@1x` variant when you publish the sheet. These should already be setup in the boilerplate `ui.tps` so that file should just be duplicated when adding a new spritesheet.


        ![spritesheet-output](http://hlf.reli.sh/assets/images/spritesheet-output.png)


        * Sheets should be set to PNG-8 (indexed) if the quality of the asset does not suffer noticeably. Otherwise PNG-32 can be used. Other texture compression settings should be researched.
    * `static`
...
#### Splashscreen
* Every project requires a splashscreen. This does not mean it has to have any visual aspect. All of the persistent/required assets are loaded on this screen. Each project’s implementation should inherit from `HLF.Splashscreen`.
