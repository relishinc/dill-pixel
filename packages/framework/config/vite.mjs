  ...
  async function generateTypes(server) {
    ...
    try {
      ...
      if (!content.trim()) {
        ...
        return { types: '/* dill-pixel.config.ts is empty, skipping augmentation. */' };
      }
      ast = parse(content, { jsx: false });
    } catch (e) {
      if (e.code === 'ENOENT') {
        // This can happen during file saves that do a delete/recreate.
        return { types: '/* dill-pixel.config.ts not found, skipping augmentation. */' };
      }
      logger.error(`[dill-pixel] Error parsing dill-pixel.config.ts: ${e.message}`);
      if (e.stack) {
        logger.error(e.stack);
      }
      return {
        types: '/* Error parsing dill-pixel.config.ts, skipping augmentation. See console for details. */',
        error: e,
      };
    }
      ...
      if (server) {
        server.ws.send({
          ...
          err: {
            ...
            stack: error.stack,
            id: path.resolve(cwd, 'dill-pixel.config.ts'),
            plugin: 'vite-plugin-dill-pixel-config',
          },
        });
      }
      return;
...
function dillPixelDevHelperPlugin() {
  return {
    name: 'vite-plugin-dill-pixel-dev-helper',
    configureServer(server) {
      server.ws.on('dill-pixel:show-error', (data) => {
        const { error } = data;
